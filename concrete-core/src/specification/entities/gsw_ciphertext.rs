...
///
/// Where $\vec{s}=\left(s_0,\cdots, s_{n-1}\right)$ and for all $0\le i <n$, we have
/// $\overline{\mathsf{ct}_i} \in \mathsf{Lev}_{\vec{s}}^{\beta, \ell}\left( -s_i \cdot
...
/// vector: $$\overline{\mathsf{ct}} = \left( \mathsf{ct}_0 , \cdots , \mathsf{ct}_{\ell-1} \right)
/// \in \mathsf{Lev}_{\vec{s}}^{\beta, \ell}\left(\mathsf{pt}\right) \subseteq
/// \mathbb{Z}_q^{(n+1)\cdot\ell}$$
///
/// Where $n=|\vec{s}|$ and for all $0\le i <\ell$, we have $\mathsf{ct}_i \in
/// \mathsf{LWE}^n_{\vec{s}}\left( \left\lfloor\mathsf{pt}\cdot \frac{q}{\beta^{i+1}} \right\rceil
...
///    \mathbb{Z}_q$
/// 	- compute $\mathsf{ct}_i \leftarrow \mathsf{LWE}.\mathsf{encrypt}\left( \mathsf{pt}_i, \vec{s}
///    ,\mathcal{D_{\sigma^2,\mu}} \right)$
