...
///
/// Where $\vec{S}=\left(S_0, \cdots , S_{k-1}\right)\in \mathcal{R}_q^k$ and for all $0\le i<k$ we
/// have $\overline{\mathsf{CT}_i} \in \mathsf{GLev}_{\vec{S}}^{\beta, \ell}\left( -S_i \cdot
/// \mathsf{PT}\right)\subseteq \mathcal{R}_q^{\ell \cdot (k+1)}$ and $\overline{\mathsf{CT}_k} \in
/// \mathsf{GLev}_{\vec{S}}^{\beta, \ell}\left( \mathsf{PT}\right)\subseteq \mathcal{R}_q^{\ell
/// \cdot (k+1)}$.
///
...
/// ###### algorithm:
/// 1. for $0\le i < k$:
/// 	- compute $\mathsf{PT}_i = -S_i\cdot\mathsf{PT} \in \mathbb{Z}_q$
/// 	- compute $\overline{\mathsf{CT}_i} \leftarrow \mathsf{GLev}.\mathsf{encrypt}\left(
///    \mathsf{PT}_i, \vec{S} ,\mathcal{D_{\sigma^2,\mu}} ,\ell \right)$
/// 2. compute  $\overline{\mathsf{CT}_n} \leftarrow \mathsf{GLev}.\mathsf{encrypt}\left(
/// \mathsf{PT}, \vec{s} ,\mathcal{D_{\sigma^2,\mu}} ,\ell \right)$ 3. output $\overline{\overline{\
/// mathsf{CT}}} = \left( \overline{\mathsf{CT}_0} , \cdots , \overline{\mathsf{CT}_{n}} \right)$
///
/// ###### equivalent algorithm (using the gadget matrix):
/// 1. for $0\le i \le k$:
/// 	- for  $0\le j < \ell$:
/// 		- compute $\mathsf{CT}_{i,j} \leftarrow \mathsf{GLWE}.\mathsf{encrypt}\left( 0, \vec{S}
///     ,\mathcal{D_{\sigma^2,\mu}} \right)$
/// 		- add to the $i$-th component of $\mathsf{CT}_{i,j}$ the value $\left\lfloor\mathsf{PT}\cdot
///     \frac{q}{\beta^{j+1}} \right\rceil \in \mathcal{R}_q$
/// 	- set $\overline{\mathsf{CT}_i} = \left( \mathsf{CT}_{i,0} , \cdots , \mathsf{CT}_{i,\ell-1}
///    \right)$
...
///
/// A GLev ciphretext is an encryption of a polynomial plaintext.
...
/// ###### algorithm:
/// 1. for $0\le i < \ell-1$:
/// 	- compute $\mathsf{PT}_i = \left\lfloor\mathsf{PT}\cdot \frac{q}{\beta^{i+1}} \right\rceil \in
///    \mathcal{R}_q$
/// 	- compute $\mathsf{CT}_i \leftarrow \mathsf{GLWE}.\mathsf{encrypt}\left( \mathsf{PT}_i, \vec{S}
///    ,\mathcal{D_{\sigma^2,\mu}} \right)$
