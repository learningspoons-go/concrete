...
///
/// ###### algorithm:
/// 1. uniformly sample each coefficient of the polynomial vector $\vec{A}\in\mathcal{R}^k_q$
/// 2. sample each integer error coefficient of an error polynomial $E\in\mathcal{R}_q$ from
/// $\mathcal{D_{\sigma^2,\mu}}$ 3. compute $B = \left\langle \vec{A} , \vec{S} \right\rangle +
