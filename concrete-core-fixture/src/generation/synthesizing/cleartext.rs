...
use crate::generation::{FloatPrecision, IntegerPrecision};
...
pub trait SynthesizesFloatCleartext<Precision: FloatPrecision, Cleartext>:
    PrototypesFloatCleartext<Precision>
where
    Cleartext: CleartextEntity,
{
    fn synthesize_cleartext(&mut self, prototype: &Self::CleartextProto) -> Cleartext;
...
mod backend_core {
    ...
    use crate::generation::synthesizing::{SynthesizesCleartext, SynthesizesFloatCleartext};
    use crate::generation::{Maker, Precision32, Precision64, PrecisionF64};
    use concrete_core::prelude::{Cleartext32, Cleartext64, CleartextF64, DestructionEngine};
    ...
    impl SynthesizesFloatCleartext<PrecisionF64, CleartextF64> for Maker {
        fn synthesize_cleartext(&mut self, prototype: &Self::CleartextProto) -> CleartextF64 {
            prototype.0.to_owned()
        }

        fn unsynthesize_cleartext(&mut self, entity: &CleartextF64) -> Self::CleartextProto {
            ProtoCleartextF64(entity.to_owned())
        }

        fn destroy_cleartext(&mut self, entity: CleartextF64) {
            self.core_engine.destroy(entity).unwrap();
        }
    }
}